...
int errno __asm__("__lintiny_errno");
...
_syscall3(int,getdents,int,fd,char*,buf,unsigned,count);

/* --- System call convenience functions. */

#ifdef __clang__
/* clang 3.4 ignores -fno-builtin-_Exit etc. specified by lintiny.
...
extern void exit(int __exitcode) __asm__("__lintiny_exit_with_fini") __attribute__((noreturn, nothrow, regparm(1)));

/* Runs destructors, but no atexit, no stdout flush, no thread sync. */
extern void _exit(int __exitcode) __asm__("__lintiny_exit") __attribute__((noreturn, nothrow, regparm(1)));
extern void _Exit(int __exitcode) __asm__("__lintiny_exit") __attribute__((noreturn, nothrow, regparm(1)));
...
#define __LINTINY_STATIC_ASSERT(name, expr_true) struct __StaticAssert##name { \
    int StaticAssert##name : (expr_true); }

typedef char int8_t;
__LINTINY_STATIC_ASSERT(SizeofInt8T, sizeof(int8_t) == 1);
typedef short int16_t;
__LINTINY_STATIC_ASSERT(SizeofInt16T, sizeof(int16_t) == 2);
typedef int int32_t;
__LINTINY_STATIC_ASSERT(SizeofInt32T, sizeof(int32_t) == 4);
__extension__ typedef long long int64_t;  /* No need for __extension__, why? */
__LINTINY_STATIC_ASSERT(SizeofInt64T, sizeof(int64_t) == 8);

typedef unsigned char uint8_t;
__LINTINY_STATIC_ASSERT(SizeofUint8T, sizeof(uint8_t) == 1);
typedef unsigned short uint16_t;
__LINTINY_STATIC_ASSERT(SizeofUint16T, sizeof(uint16_t) == 2);
typedef unsigned int uint32_t;
__LINTINY_STATIC_ASSERT(SizeofUint32T, sizeof(uint32_t) == 4);
__extension__ typedef unsigned long long uint64_t;  /* No need for __extension__, why? */
__LINTINY_STATIC_ASSERT(SizeofUint64T, sizeof(uint64_t) == 8);
